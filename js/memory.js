// MEMORY.JS - FIREBASE EDITIE (Volledig & Gecorrigeerd)
...
const memThemes = {
    ...
    'sonic':     { locked: false, extension: 'png', path: 'assets/images/memory/natuur/' }, // Let op mapnaam
    ...
    <div class="memory-setup">
        <div class="setup-group">
            ...
            <div class="name-row">
                ...
                <button class="player-btn" onclick="memSelectPerson('Vince', this)">👩🏽‍🦱<br>Vince</button>
                <button class="player-btn" onclick="memSelectPerson('Fran', this)">👩🏻<br>Fran</button>
            </div>
...
function memAddCustomPerson() { 
    const i = document.getElementById('mem-custom-name'); 
...
function memRenderPalette() { 
    const cp = document.getElementById('mem-color-palette'); 
    if(!cp)return; 
...
function memSelectColor(c) { 
    ...
    const inp = document.getElementById('mem-custom-name'); 
    if(inp) inp.placeholder="Naam..."; 
...
function startMemoryGame() {
    ...
    memoryState.maxStreakP1 = 0; 
    memoryState.maxStreakP2 = 0;
    ...
    if(isMix) { 
        ...
        Object.keys(memThemes).forEach(tName => { 
            ...
            if(!tData.locked && !tData.isMix) { 
                for(let i=1; i<=15; i++) { allPool.push({ src: `${tData.path}${i}.${tData.extension}`, cover: mixCover }); } 
            } 
        }); 
...
function memUpdateActiveBadgeColor() { 
    memoryState.playerNames.forEach((p, i) => { 
        ...
        if(b) { 
            ...
            if(active) { b.style.borderColor = p.color; b.style.boxShadow = `0 0 15px ${p.color}`; } 
            else { b.style.borderColor = 'transparent'; b.style.boxShadow = 'none'; } 
        } 
    }); 
}
